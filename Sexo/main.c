...
void localizar_palavra(Tabela *tab[]){
    char palavra[30];
    fgets(palavra, 30, stdin);
    unsigned int chave=hashcode(palavra);

    if(tab[chave]->quantidade==0){
      printf("\nNao existe tal palavra\n");
      return;
    }
    else{
      printf("\nPalavra %s apareceu %d vezes e esta na posicao %d", tab[chave]->palavra,tab[chave]->quantidade, chave);
      return;
    }
...
int o=5;
    ...
    {
      printf("\n----MENU----\n");
	    printf("\n1- Prucurar uma palavra\n2- Sair\n");
      scanf("%d", &o);
      if(o==2) break;
      else if(o==1){
        localizar_palavra(hash_table);
      }
      else{
        printf("\nOpcao Invalida\n");
      }
